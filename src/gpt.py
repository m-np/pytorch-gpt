...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads, head_size):
        ...
        self.projs = nn.Linear(n_embed, n_embed)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        out = torch.cat([h(x) for h in self.heads], dim=-1) # Concatenate at channel dim
        out = self.dropout(self.projs(out))
        return out


class FeedForward(nn.Module):
    """A Simple linear feedforward NN"""
